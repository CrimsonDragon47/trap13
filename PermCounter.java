...
public class PermCounter implements Runnable{
  ...
  private class LoopIterator implements Runnable{
    ...
    public void run(){
      ...
      for(int i0=threadId;i0<size;i0+=nThreads){
        out.printf("----->%s %d%%%n",t.getName(),(100*(i0-threadId))/size);
  ...
  private class PreList{
    ...
    private class Worker implements Runnable{
      ...
      @Override
      public void run(){
        out.printf("%s started. %d%n", t.getName(), preList.size());
        int size = preList.size();
        for(int i0=threadId;i0<size;i0+=nThreads){
          out.printf("-----> %s: %d%%%n",t.getName(),(100*i0)/size);
          for(int i1=0;i1<size;i1++){
            for(int i2=0;i1<size;i1++){
              for(int i3=0;i1<size;i1++){
