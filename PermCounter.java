...
public class PermCounter implements Runnable{
  ...
  public static void start(PrintStream out, int nThreads){
    ...
    if(totalCount==checksum){
      ...
      out.printf("---LOWER---%nNum: %s%nDen: %s%n=%d%n",numL,den,den.divide(numL));
      //out.printf("---UPPER---%nNum: %d%nDen: %d%n=%.9f%n",numP,den,numP/(double)den);
      
      //TODO checksum2
      den=den.divide(new BigInteger("331776"));
      den=den.subtract(new BigInteger(Long.toString(checksum)));
      double x = (den.doubleValue()+1)/1.0;
  ...
  private class LoopIterator implements Runnable{
    ...
    public void run(){
      ...
      for(int i0=threadId;i0<size;i0+=nThreads){
        if(i0-threadId%step==0) out.printf("%n----->%s %d%%%n",t.getName(),(100*(i0-threadId))/size);
        ...
        for(int i1=0;i1<size;i1++){
          ...
          for(int i2=0;i2<size;i2++){
            for(int i3=0;i3<size;i3++){
              ...
              BigInteger loopCount = new BigInteger(Long.toString(l[0].count));
              for(int i=1;i<4;i++) loopCount = loopCount.multiply(new BigInteger(Long.toString(l[i].count)));
              //den+=loopCount*24*24*24*24;
              den=den.add(new BigInteger("331776").multiply(loopCount));
              long count=0;
              ...
              if((count=isPossible(l[0].seq, l[1].seq, l[2].seq, l[3].seq))>0){
                ...
                numL=numL.add(new BigInteger(Long.toString(count)).multiply(loopCount));
                //numP+=count;
                /*
                if(count==331776){
