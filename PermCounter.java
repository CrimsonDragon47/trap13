    ...
    private class Counter<T extends Element>{
        ...
        private class Worker<T extends Element> implements Runnable{
            ...
            private void count(Element e0, Element e1, Element e2, Element e3){
                int x=5;
                int orders=1;
                if(e0.fp==e1.fp) orders*=--x;
                if(e1.fp==e2.fp) orders*=--x;
                if(e2.fp==e3.fp) orders*=--x;
                                
                BigInteger permCount = new BigInteger(Integer.toString(orders));
                permCount=permCount.multiply(new BigInteger(Long.toString(e0.count)));
                ...
                if((count=isPossible(e0.seq,e1.seq,e2.seq,e3.seq))>0){
                    ...
                    BigInteger subCount = new BigInteger(Integer.toString(orders));
                    subCount = subCount.multiply(new BigInteger(Long.toString(e0.subCount)));
