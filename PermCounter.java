...
public class PermCounter{
    ...
    private class Counter<T extends Element>{
        ...
        private class Worker<T extends Element> implements Runnable{
                    ...
                    for(int i1=0;i1<size;i1++){
                        for(int i2=0;i2<size;i2++){
                            for(int i3=0;i3<size;i3++){
                                ...
                                T t2 = perms.get(i2);
                                T t3 = perms.get(i3);
                                BigInteger permCount = new BigInteger(Long.toString(t0.count));
                                ...
                                if((count = isPossible(t0.seq,t1.seq,t2.seq,t3.seq))>0){
                                    ...
                                    if(t0.sub!=null) doSub(t0.sub,t1,t2,t3);
                                    //sub = t1;
                                    if(t1.sub!=null) doSub(t1.sub,t0,t2,t3);
                                    //sub = t2;
                                    if(t2.sub!=null) doSub(t2.sub,t0,t1,t3);
                                    //sub = t3;
                                    if(t3.sub!=null) doSub(t3.sub,t0,t1,t2);
                                    
                                }else{
                                    //TODO den sub counts 
                                    BigInteger subCount = new BigInteger(Long.toString(t0.subCount));
                                    subCount = subCount.multiply(new BigInteger(Long.toString(t1.subCount)));
                                    subCount = subCount.multiply(new BigInteger(Long.toString(t2.subCount)));
                                    subCount = subCount.multiply(new BigInteger(Long.toString(t3.subCount)));
            ...
            private void doSub(Element sub, T t0, T t1, T t2){
                BigInteger permCount = new BigInteger(Long.toString(sub.count));
                permCount=permCount.multiply(new BigInteger(Long.toString(t0.count)));
                permCount=permCount.multiply(new BigInteger(Long.toString(t1.count)));
                permCount=permCount.multiply(new BigInteger(Long.toString(t2.count)));
                den=den.add(permCount.multiply(new BigInteger("331776")));
                long count;
                if((count=isPossible(sub.seq,t0.seq,t1.seq,t2.seq))>0){
                    num=num.add(permCount.multiply(new BigInteger(Long.toString(count))));
                    
                    if(sub.sub!=null) doSub(sub.sub,t0,t1,t2);
                }else{
                    BigInteger subCount = new BigInteger(Long.toString(sub.subCount));
                    subCount = subCount.multiply(new BigInteger(Long.toString(t0.count)));
                    subCount = subCount.multiply(new BigInteger(Long.toString(t1.count)));
                    subCount = subCount.multiply(new BigInteger(Long.toString(t2.count)));
                    den = den.add(subCount.multiply(new BigInteger("331776")));
                }
            }
        }
