...
public class Sequencer{
  ...
  public void run(){
   ...
   List<Element> sequences = new ArrayList();
   ...
   do{
     ...
     byte[] sequence = getSequence(combination);
     
     
     
     Element e = new Element(sequence);
     int index = sequences.indexOf(e);
     if(index>=0){
       sequences.set(index, e.setCount(sequences.get(index).count+1));
     }else{
       sequences.add(e);
     }
   ...
   output.writeObject(sequences.toArray());
